...
from compass_backend.contracts.answer_layer import AnswerLayerMode


def _default_academic_year() -> str:
    """W1-06 (#844): proxy import to keep the canonical helper as the only place
    a year literal lives. Import is local to avoid pulling planning into
    every config consumer at module-load time."""

    from compass_backend.planning.temporal import default_current_academic_year

    return default_current_academic_year()
...
class Settings(BaseSettings):
    ...
    current_academic_year: str = Field(
        # W1-06 (#844): single source of truth lives in planning.temporal.
        # Settings binds at class-load time via default_factory so the
        # literal lives in exactly one module.
        default_factory=_default_academic_year,
    ...
    pg_pool_max_inactive_lifetime: float = Field(
        # Issue #1122: asyncpg has no pre-ping. An idle connection over the
        # Tailscale tunnel to staging Postgres can be NAT-reclaimed and then
        # handed out dead, surfacing as an intermittent 500 (live path) or a
        # `ConnectionDoesNotExistError`/connect-timeout (sweep path). Closing
        # connections idle longer than this lifetime forces a fresh, healthy
        # connect instead of handing out a reclaimed one. 300s matches
        # asyncpg's own default; lower it (env PG_POOL_MAX_INACTIVE_LIFETIME)
        # when the NAT idle window on the path is shorter.
        default=300.0,
        gt=0,
        description=(
            "Seconds an asyncpg pool connection may sit idle before it is "
            "closed and recycled (guards against NAT-reclaimed stale "
            "connections over Tailscale)."
        ),
    )
