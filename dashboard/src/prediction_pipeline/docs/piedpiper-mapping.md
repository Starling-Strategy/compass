...
How our `prediction_pipeline/` maps to the [PiedPiper](https://github.com/Nathan-Roll1/PiedPiper) repo.

## File-by-File

| Our File | PiedPiper Equivalent | Notes |
|---|---|---|
...
| `retriever.py` | `PredictionPipeline._build_vespa_query()` | Vespa hybrid search + Postgres fallback. `build_search_query()` matches PiedPiper's rich query building |
| `predict.py` | `PredictionPipeline._predict_single()` | PydanticAI structured output vs PiedPiper's raw Gemini + JSON mode. INA footer enforces the silence rule |
...
| `db.py` | `database.py` | Raw psycopg2 vs PiedPiper's SQLAlchemy ORM |
| `run.py` | `PredictionPipeline.run()` | Functional `run_pipeline()` with argparse CLI |

## Concept Mapping

| PiedPiper Concept | Our Implementation |
|---|---|
...
| AY23 fallback | **NOT implemented** (violates the silence rule). Instead: INA audit trail in `run.py` logs when prior year had a value |
...
| PiedPiper Label | Our Label | Meaning |
|---|---|---|
...
| _(implicit)_ | INA_FALSE_NEG | Predicted value, golden is INA (**silence-rule violation**) |
| _(implicit)_ | NOGOLDEN | No golden answer available |

## Key Differences from PiedPiper

1. **AI client**: PydanticAI structured output vs raw Gemini + JSON mode + manual parsing
2. **Silence-rule enforcement**: INA footer with answer-type-specific guidance (PiedPiper's dropdown guidance says "prefer the option"; we say "INA always wins")
3. **No AY23 fallback**: The client explicitly rejected this. We log for analyst review instead
4. **Type-aware evaluation**: 0.5% numeric tolerance, order-independent multi-select comparison
5. **Evaluation snapshots**: Save/load/compare for regression testing (PiedPiper doesn't have this)
