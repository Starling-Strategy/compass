...
def _boolean_value_key(value: object) -> str:
    """Normalize a data cell to the key form the boolean vocabularies use."""

    return str(value).casefold().replace(",", "").strip().strip(".")


def is_boolean_answer_token(value: object) -> bool:
    """Return True when ``value`` is a bare yes/no-style boolean token.

    Distinguishes a yes/no filter comparison value (``"yes"``, ``True``,
    ``"no"``) from a specific category label (``"Permitted"``, ``"annual"``).
    Used by the count text-equality predicate to decide when to compare by
    affirmative/negative equivalence vs. exact string equality (#129). Reads a
    data cell value, never user prose (validator-data-filter role under the
    no-prose-dispatch doctrine).
    """
    ...
    if value is None:
        return False
    key = _boolean_value_key(value)
    return key in _BOOLEAN_TRUE_VALUES or key in _BOOLEAN_FALSE_VALUES


def categorical_answer_is_affirmative(value: object) -> bool:
    """Classify a covered categorical policy answer as affirmative.

    A recognized negative sentinel (``No``, ``None``, ``N/A``,
    ``"issue not addressed"``, a ``"not ..."`` phrase) or a non-positive number
    is negative; every other non-empty covered value is affirmative — including
    qualified affirmatives (``"Yes (for teachers hired after 2015)"``,
    ``"Offered - district-funded"``) and categorical affirmatives
    (``"Permitted"``, ``"Required"``, an eligibility list). This inverts the old
    prefix-whitelist default that silently bucketed any unrecognized affirmative
    as negative (#123 / #129).

    Reads a data cell value, never user prose — a validator-data-filter role
    under the no-prose-dispatch doctrine (see tests/test_no_prose_dispatch.py).
    """
    ...
    if isinstance(value, int | float):
        return value > 0
    key = _boolean_value_key(value)
    if not key or key in _BOOLEAN_FALSE_VALUES:
        return False
    if key in _BOOLEAN_TRUE_VALUES:
        return True
    if key.startswith("not "):
        return False
    number = parse_compass_numeric_value(value)
