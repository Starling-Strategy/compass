...
| --- | --- | --- |
| **Unique visitors** | Site-level unique visitors from Google Analytics, for people who viewed Compass on Pathfinder. GA de-duplicates by client ID across all time, so every window — including all-time — returns a meaningful number | A self-hosted Umami instance remains in the code as a fallback, but GA is the configured primary source and the one these numbers come from. Retiring the fallback is tracked in [§9](09-known-issues-and-limitations.md#two-analytics-tools-measure-the-same-thing-google-analytics-should-be-the-only-one). |
