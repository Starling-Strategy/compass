...
| **Metric Calculator** | NCTQ's internal analyst-review tool (and its upstream prediction engine, PiedPiper) for turning district policy documents into reviewed data. It is not part of Compass; approved answers reach Compass only indirectly, through NCTQ's published TCD/Pathfinder database. | [Metric Calculator reference](metric-calculator.md) |
